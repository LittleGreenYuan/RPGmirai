...
import net.mamoe.mirai.utils.ExternalResource.Companion.sendAsFileTo
...
object WithoutConfiguration {
    ...
    fun main(args: Array<String>): Unit = runBlocking {
        ...
        val UserKeyListPath = WorkPath+"UserKey.txt"
        val LogUserKeyListPath = WorkPath
        ...
        val Mapimg = ImageIO.read(File(WorkPath+"data\\IMG01AT.png")).getScaledInstance(400, 400, ImageShaper.SCALE_SMOOTH)
        val flagimg = ImageIO.read(File(WorkPath+"data\\flage.png")).getScaledInstance(30, 30, ImageShaper.SCALE_SMOOTH)
        //val sizeImage = flagimg.getScaledInstance()
                                                    ...
                                                    if(message.contentToString().contentEquals("/raidattack")){
                                                        ...
                                                        if(message.contentToString().contentEquals("/bag")){
                                                            ...
                                                            if(message.contentToString().startsWith("/sg")){
                                                                ...
                                                                if(message.contentToString().startsWith("/imgxxx")){
                                                                    ...
                                                                    for (layer in MapArr) {
                                                                        drawArr.add(
                                                                            DrawItem(
                                                                                type = layer,
                                                                                x = if (layer == "F") 250 else 0,
                                                                                y = if (layer == "F") 300 else 0,
                                                                    ...
                                                                    /*
                                                                    val fileName=WorkPath+"lio.mp3"
                                                                    val filex = File(fileName).toExternalResource()
                                                                    val ExternalResource=subject.uploadAudio(filex)
